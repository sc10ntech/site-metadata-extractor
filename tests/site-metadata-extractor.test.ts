import fs from "fs";
import siteMetadataExtractor, { lazy } from "../src/index";
...
const checkFixture = (site: string, fields = allFields) => {
  ...
  const data = siteMetadataExtractor(html, orig.url);
...
describe("Site Metadata Extractor", () => {
  it("exists", () => {
    expect(siteMetadataExtractor).toBeDefined();
  });
